...
- **系統狀態列**：標題列常駐指示（Ollama／模型／MCP／資料庫／Python），啟動與儲存設定後自動健檢。
  點開可看逐項狀態與可行動提示、重新檢查、一鍵前往設定；有問題時聊天頁上方跳橫幅，失敗不再靜默。
- **深色模式**：標題列主題切換（跟隨系統／淺色／深色），記憶於設定，`system` 模式跟著作業系統。
- **命令面板（`⌘K` / `Ctrl+K`）**：搜尋所有指令——新對話、切換分頁、匯出、重連 MCP、重新健檢、
  主題、語言——並可**跳到對話**。
- **鍵盤快捷**：`⌘N` 新對話 · `⌘1–5` 切換分頁 · `⌘,` 設定 · `⌘E` 匯出 · `Esc` 停止生成／關閉面板。
- **HITL 核可框**：`Enter` 核可、`Esc` 拒絕、焦點鎖定、工具參數美化呈現。
- **稽核強化**：搜尋＋篩選（全部／已核可／已拒絕／錯誤）、點列展開完整參數、結果摘要與 session id、
  並可把日誌匯出成 `.jsonl`。
- **全域 toast**（已儲存／匯出／重連／刪除）、智慧自動捲動＋「捲到最新」鈕、錯誤泡泡可重試。
- **視窗記憶與可收合側欄**：視窗記住大小／位置；對話清單可收合讓對話區變寬——用聊天標頭的面板鈕，
  或點側邊「對話」圖示（已在對話頁時再點一次即收合／展開）。
- **示範題**：開新對話即有一鍵示範題，每題都會觸發真實工具（檢傷生命徵象、SOAP 擴寫、資料查詢，
  以及一題故意查無的紀錄以示範不亂編）。
- **行內說明**：每個設定旁都有小小的 `[?]`，hover 或鍵盤聚焦即顯示白話說明，講清楚該選項在做
  什麼、為什麼這樣設（取樣溫度、工具回合上限、唯讀系統提示、Ollama／Python／資料庫路徑、字級…）。
- **關於頁**：說明定位、隱私核心、三層防線與團隊——並含**命名由來**，把精神直接藏進名字裡
  （**Note**＝病歷紀錄、**Sentry**＝把關的哨兵，正是人類覆核這道防線，也暗示資料被看守在本機）。
...
**應用**分頁把自由聊天升級成四種結構化、表單驅動的流程——對應四支 MCP 支柱。每個表單會組出
一段「接地」提示，再走同一套 agent 迴圈——因此每次工具呼叫仍會請你核可，結果也會落在一般
對話串中可繼續延伸。
...
- **應用 C · 用藥安全**：輸入目前用藥清單與已知過敏；LLM 呼叫 `check_drug_interactions` 與
  `check_allergy_conflict`，彙整風險等級與處置建議。最終由醫師與藥師確認。
- **應用 D · 生命徵象 → FHIR**：輸入生命徵象；LLM 呼叫 `vitals_to_fhir`，輸出 HL7 **FHIR R4**
  的 `Observation` 資源（LOINC 代碼、UCUM 單位）——示範跨系統的標準化交換。

**不再面對空白表單。** 每個表單都有兩種一鍵帶入方式：

- **載入範例**：內建逼真 presets——即時、demo 安全。再點一次會循環到下一個案例。
...
除了原始日誌，NoteSentry 還能把一段工作產出兩種**確定性、自包含**的交付物——由程式組裝
（非 LLM 生成），因此可重現、不含任何臆造內容。兩者都透過原生「另存新檔」匯出（預設開在專案
`exports/` 並記住上次位置），可存成 Markdown **或**可列印、配品牌色的 **HTML** 文件。

- **個案報告**（單一對話，從聊天標頭或 `⌘K` →「個案報告」）：從對話串組出的臨床交班文件——
  輸入、助理的評估、每次工具呼叫，以及結構化結果表：由生命徵象渲染的 **FHIR R4 `Observation`**
  表（LOINC／UCUM）、藥物**交互作用／過敏衝突**表，以及該對話完整的**稽核軌跡**
  （時間 · 工具 · 參數 · 核可／拒絕）與核可／拒絕總計。
- **治理稽核報表**：對整份稽核日誌的組織層級彙整——KPI（工具呼叫總數、**HITL 核可率**）、
  各 server／各工具分布、涵蓋時間區間——作為 AI 治理與可問責的佐證。

> 報表一律不含原始臨床全文，只彙整結構化的工具結果與稽核軌跡，因此符合 DUA、可安全分享。

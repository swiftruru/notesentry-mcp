...
- **Inline help**: every setting carries a small `[?]` icon — hover or keyboard‑focus it for a
  plain‑language explanation of what the option does and why (sampling temperature, the tool‑turn
  cap, the read‑only system prompt, the Ollama/Python/DB paths, font size…).
- **About page**: app purpose, the privacy core, the three‑layer defense, and the team — plus a
  *naming rationale* that spells out the philosophy in the name itself (**Note** = clinical notes,
  **Sentry** = the gatekeeper that is the human‑review line, with the data kept on‑device).
...
Beyond raw logs, NoteSentry turns a session into two **deterministic, self‑contained**
deliverables — assembled in code (not generated by the LLM), so they are reproducible and contain
no fabricated content. Both export through a native Save As dialog (defaults to the project
`exports/` folder and remembers the last location) as Markdown **or** a printable, brand‑styled
**HTML** document.

- **Case report** (per conversation, from the chat header or `⌘K` → *Case report*): a clinical
  hand‑off document built from the thread —
  the inputs, the assistant's assessment, every tool call, and structured result tables: a
  **FHIR R4 `Observation`** table (LOINC/UCUM) rendered from vitals, drug **interaction /
  allergy‑conflict** tables, and the conversation's full **audit trail** (time · tool · arguments ·
  approved/rejected) with approve/reject totals.
- **Governance & audit report**: an organization‑level summary over the whole audit log — KPIs
  (total tool calls, **HITL approval rate**), a per‑server / per‑tool breakdown, and the covered
  time period — evidence for AI governance and accountability.

> Reports never include raw clinical free‑text; they only summarize structured tool results and the
> audit trail, so they stay DUA‑safe to share.
